...
impl<'a> Tokenizer<'a> {
    ...
    fn at_keyword(&self) -> Option<RawToken> {
        ...
        match self.stream.idx - self.current_start {
            ...
            3 if ident == b"mut" => Some(RawToken::Keyword(Keyword::Mut)),
            ...
            5 if ident == b"Never" => Some(RawToken::Keyword(Keyword::Never)),
            5 if ident == b"Tuple" => Some(RawToken::Keyword(Keyword::Tuple)),
            5 if ident == b"alias" => Some(RawToken::Keyword(Keyword::Alias)),
            ...
            5 if ident == b"trait" => Some(RawToken::Keyword(Keyword::Trait)),
            ...
            6 if ident == b"Option" => Some(RawToken::Keyword(Keyword::Option)),
            6 if ident == b"String" => Some(RawToken::Keyword(Keyword::String)),
            6 if ident == b"delete" => Some(RawToken::Keyword(Keyword::Delete)),
            6 if ident == b"object" => Some(RawToken::Keyword(Keyword::Object)),
            ...
            8 if ident == b"__Option" => Some(RawToken::Keyword(Keyword::UnsafeOption)),
    ...
    fn number(&mut self, start: char) -> Res<RawItem> {
        if start != '.' {
            if let Some(next) = self.stream.next_char() {
                ...
                self.gen_number(NumberKind::DecI)
            }
        } else {
            self.punct(start)
        }
    ...
    fn dec_number(&mut self, seen_point: bool, mut prev_char: char) -> Res<RawItem> {
        ...
        self.check_trailing_underscore(prev_char)?;
        if is_f {
            self.gen_number(NumberKind::DecF)
        } else {
            self.gen_number(NumberKind::DecI)
        }
    }
